...
public class DBHelper1 extends SQLiteOpenHelper{
    ...
    public void delete_todo2(String id, String date) {
        SQLiteDatabase db = getWritableDatabase();

        db.execSQL("DELETE FROM TODODATA WHERE id='" + id + "'AND date = '" + date+ "';");
        db.close();
    }
    ...
    public int get_ordernum(String time, String id) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM TODODATA", null);
        String a = null;
        while (cursor.moveToNext()) {
            if (cursor.getString(1).equals(time) && cursor.getString(0).equals(id)) {
        ...
        return Character.getNumericValue(a.charAt(2));

    }

    public void update_achieve(String time, String id,int achieve){
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("UPDATE TODODATA SET achieve='" + achieve + "' WHERE id='" + id + "'AND date = '" + time +"' ;");
        db.close();

    }

    public String[] get_todo(String id){
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM TODODATA", null);
        ...
        while(cursor.moveToNext()){
            if(cursor.getString(0).equals(id)) {
                a[i] = cursor.getString(1) + "  " + cursor.getString(2) + " " + cursor.getString(3)+ " ";
                i++;
            }
        }
    ...
    public int count_todo(String id){
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM TODODATA", null);
        int i=0;
        while(cursor.moveToNext()){
    ...
    public int[] get_achieve(String id){
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM TODODATA", null);
        int a[] = new int[100];
        int i =0;
        while(cursor.moveToNext()){
            if(cursor.getString(0).equals(id) & cursor.getInt(4)==1){
               a[i]=1;
               i++;
            }
            else if(cursor.getString(0).equals(id) & cursor.getInt(4)==0){
                a[i]=0;
                i++;
            }
